...
async function main() {
    ...
    ret = await preparedAttack(INPUT_TOKEN_ADDRESS, out_token_address, amount, level); 
    if(ret == false) {
        process.exit();
    }
...
async function getPoolInfo(input_token_address, out_token_address){
    ...
    try{
        ...
        pool_info = {'klast': kLast, 'contract': pool_contract, 'forward': forward, 'input_volumn': bnb_balance, 'output_volumn': token_balance}
...
async function getTokenInfo(tokenAddr, token_abi_ask) {
    try{
        ...
        console.log('Unable to read Token Info, may due to a Proxy Smart contract');
        return false;
    }
}

async function preparedAttack(input_token_address, out_token_address, amount, level)
{
    ...
    //check pool info
    if(await getPoolInfo(WBNB_TOKEN_ADDRESS, out_token_address) == false)
        return false;
